...
import './styles.css';
import { useChatViewport } from './viewport';
...
function Inbox({ openChat }: {
    openChat: (id: string) => void;
}) { const { t } = useTranslation(); const q = useQuery({ queryKey: ['conversations'], queryFn: () => api<any[]>('conversations'), refetchInterval: 15000 }); return <section><h1>{t('chats')}<span className="accent">.</span></h1>{q.isLoading ? <Loading /> : q.error ? <Empty title={t('error')}/> : !q.data?.length ? <Empty title={t('noChats')}/> : q.data.map(c => <button className="chat-row" key={c.id} onClick={() => openChat(c.id)}><Portrait url={c.other?.photos[0]?.url} alt={c.other?.displayName || ''}/><div className="chat-copy"><h3>{c.other?.displayName}</h3><p>{c.last?.body || t(c.last?.photoId ? 'photo' : 'startChat')}</p></div>{c.last && (!c.readAt || c.last.createdAt > c.readAt) && <span className="unread"/>}<ArrowUpRight /></button>)}</section>; }
function Chat({ id, me, notice, onClose }: {
    ...
    useEffect(() => { void api(`conversations/${id}/read`, 'POST').catch(() => { }); if (nearBottom.current || q.data?.items?.[0]?.senderId === me.id) { const panel = scroll.current?.parentElement; if (panel) panel.scrollTop = panel.scrollHeight; } }, [id, q.data?.items?.[0]?.id]);
    ...
    async function attach(file: File) { if (busy) return; setBusy(true); try {
